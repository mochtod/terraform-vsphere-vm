resource "vsphere_virtual_machine" "vm" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "    echo \"$(date '+%Y-%m-%d %H:%M:%S') - CURL API CALL: ${var.chr_api_server}/api/v2/chr/register\" | tee -a /var/log/chr_registration/main.log",
      ...
      "    echo \"Full API URL: ${var.chr_api_server}/api/v2/chr/register\" | tee -a /var/log/chr_registration/main.log",
      ...
      "      -X POST \\",
      "      '${var.chr_api_server}/api/v2/chr/register' \\",
      ...
      "      echo 'Sending API status request to ${var.chr_api_server}/api/v2/status' | tee -a /var/log/chr_registration/main.log",
      "      echo \"$(date '+%Y-%m-%d %H:%M:%S') - CURL STATUS CHECK: ${var.chr_api_server}/api/v2/status\" | tee -a /var/log/chr_registration/main.log",
      
      "      # Check the API endpoint with a more stable approach",
      "      echo \"Testing API endpoint: ${var.chr_api_server}/api/v2/status\" | tee -a /var/log/chr_registration/main.log", 
      "      curl -s --connect-timeout 10 --max-time 30 --insecure \\",
      "        --resolve \"$CHR_HOSTNAME:443:$CHR_IP\" \\",
      "        -u \"$CHR_USERNAME:$CHR_PASSWORD\" \\",
      "        '${var.chr_api_server}/api/v2/status' > /tmp/chr_status.json",
      ...
      "        echo 'Check API endpoint path: ${var.chr_api_server}/api/v2/status' | tee -a /var/log/chr_registration/main.log",
