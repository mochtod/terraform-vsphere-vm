resource "vsphere_virtual_machine" "vm" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "  ",
      "  # First, backup existing resolv.conf",
      "  sudo cp /etc/resolv.conf /etc/resolv.conf.backup.$(date +%s)",
      "  ",
      "  # Add CHR corporate DNS servers to resolv.conf",
      ...
      "    cat /etc/resolv.conf.backup.$(ls -t /etc/resolv.conf.backup.* | head -1 | cut -d. -f4) | grep -v '^nameserver' | grep -v '^search'",
      "  } | sudo tee /etc/resolv.conf > /dev/null",
      "  ",
      "  # Extract hostname and IP from CHR API server URL",
      ...
      "  ",
      "  # Add retry logic for the registration",
      "  RETRY_COUNT=0",
      "  MAX_RETRIES=3",
      ...
      "    ",
      "    # Use curl with explicit IP resolution to avoid DNS issues",
      ...
      "      ",
      "      # Additional debugging for API connectivity",
      "      echo 'Testing direct connectivity to CHR API...'",
      "      curl -sS --connect-timeout 10 --max-time 30 --insecure \\",
      "        --resolve \"$CHR_HOSTNAME:443:$CHR_IP\" \\",
      "        '${var.chr_api_server}/api/status' || echo 'API connectivity test failed'",
      "    fi",
      "    ",
      ...
      "    # Log the issue but don't fail the deployment",
